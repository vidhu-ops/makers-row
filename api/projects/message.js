...
module.exports = async function handler(req, res) {
  ...
  try {
    const auth=await authenticate(req); if(!isAdmin(auth)){res.status(403).json({error:'Only the admin can send CRM updates.'});return;}
    ...
    const { response, payload } = await supabaseRequest('project_messages', {
      ...
      body: JSON.stringify({ project_id: body.project_id, sender_role: body.sender_role || 'designer', sender_name: body.sender_name || 'Get It Done designer', message: body.message })
    });
